<%@ Page Title="" Language="C#" MasterPageFile="~/Masters/Ext/Site.Master" AutoEventWireup="true" CodeBehind="RolTree.aspx.cs" Inherits="Aim.Portal.Web.Modules.SysApp.OrgMag.RolTree" %>
    ...
    function setPgUI() {
        ...
        var tlBar = new Ext.Toolbar({
            ...
            tree.on('beforeload', function(node) {
                if (node.attributes["Type"] == "RType") {
                    tree.loader.dataUrl = 'RolTree.aspx?asyncreq=true&reqaction=querydescendant&type=rtype&id=' + node.attributes.RoleTypeID;
                }
            });
        ...
        function adjustData(jdata) {
            if ($.isArray(jdata)) {
                $.each(jdata, function() {
                    this.ID = this.RoleID || this.RoleTypeID;
                    if (this.RoleTypeID) {
                        this.Type = "RType";
                        this.id = "RT_" + this.ID;
                        this.text = this.Name;
                        this.leaf = !this.HasRole;
                    } else if (this.RoleID) {
                        this.id = this.ID;
        ...
        function saveChanges() {
            ...
            $.each(allNodes, function() {
                var node = this;
                var cID = node.attributes.RoleID;
