<%@ Page Title="动态角色权限" Language="C#" MasterPageFile="~/Masters/Ext/Site.Master" AutoEventWireup="true" CodeBehind="DPermissionRole.aspx.cs" Inherits="Aim.Portal.Web.Modules.SysApp.SiteMag.DPermissionRole" %>
...
<script type="text/javascript">
			...
			store = new Ext.ux.data.AimJsonStore({
			    ...
			    proxy: new Ext.ux.data.AimRemotingProxy({
			        aimbeforeload: function(proxy, options) {
			            var rtype = Ext.getCmp("roleCombo").getValue();
			            options.data = { rtype: rtype, id: id, op: op };
			        }
			    })
			});
    ...
    function buildGrid() {
        ...
        var tlBar = new Ext.ux.AimToolbar({
            ...
            }, '-', { text: '添加角色', iconCls: 'aim-icon-user-add',
                handler: function() {
                    var cid = Ext.getCmp("roleCombo").getValue();
                    openMdlWin("/CommonPages/Select/RolSelect/MRolSelect.aspx?rtntype=array&cid=" + cid, "add");
                }
            }, { text: '移除角色', iconCls: 'aim-icon-user-delete',
                handler: function() {
    ...
    function linkRender(val, p, rec) {
        ...
        switch (this.dataIndex) {
            case "Name":
                rtn = "<a class='aim-ui-link' onclick='openMdlWin(\"/Modules/SysApp/OrgMag/RolEdit.aspx?id=" + rec.json.Data + "\", null, usrEditStyle)'>" + val + "</a>";
                break;
        }
...
<asp:Content ID="BodyContent" ContentPlaceHolderID="BodyHolder" runat="server">
    <div id="header" style="display:none;"><h1>动态角色权限列表</h1></div>
</asp:Content>
