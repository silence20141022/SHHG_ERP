<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="GuoChanYaSuoJi_Purchase.aspx.cs"
    Inherits="Aim.Examining.Web.PurchaseManagement.GuoChanYaSuoJi_Purchase" %>
    ...
    <script type="text/javascript">
        ...
        Ext.onReady(function () {
            ...
            store = Ext.create('Ext.data.JsonStore', {
                ...
                proxy: {
                    url: 'GuoChanYaSuoJi_Purchase.aspx?action=load',
                    type: 'ajax',
                    reader: {
                        type: 'json',
                        root: 'rows',
                ...
                listeners: { beforeload: function (store, options) {
                    var new_params = {
                        ...
                        limit: Ext.getCmp('pagesize').getValue()
                    }
        ...
        function displayInnerGrid(div) {
            var store_inner = Ext.create('Ext.data.JsonStore', {
                fields: ['Id', 'Name', 'Code', 'Quantity', 'BuyPrice', 'ProductId', 'PCN', 'Amount', 'RuKuDanQuan', 'FuKuanDanQuan', 'KaiPiaoQuan'],
                proxy: {
                    url: 'GuoChanYaSuoJi_Purchase.aspx?action=loaddetail&id=' + div,
                    type: 'ajax',
                    reader: {
                        type: 'json',
                        root: 'innerrows'
            ...
            var grid_inner = Ext.create('Ext.grid.Panel', {
                ...
                columns: [
                    ...
                    { header: '单价', dataIndex: 'BuyPrice', width: 70 },
                    { dataIndex: 'Quantity', header: '采购数量', width: 70 },
		            { dataIndex: 'Amount', header: '金额', width: 80 },
                    { dataIndex: 'RuKuDanQuan', header: '已生成入库单数量', width: 120 },
                    { dataIndex: 'FuKuanDanQuan', header: '已生成付款单数量', width: 120 },
                    { dataIndex: 'KaiPiaoQuan', header: '已开票数量', width: 120 }
                    ],
                renderTo: div
            })
        }
