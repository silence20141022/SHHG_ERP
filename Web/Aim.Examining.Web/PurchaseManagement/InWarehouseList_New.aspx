    ...
    <script type="text/javascript">
        ...
        Ext.onReady(function () {
            ...
            store = Ext.create('Ext.data.JsonStore', {
                ...
                listeners: { beforeload: function (store, options) {
                    var new_params = { InWarehouseNo: Ext.getCmp("InWarehouseNo_s").getValue(),
            ...
            var grid = Ext.create('Ext.grid.Panel', {
                ...
                listeners: { cellclick: function (tableview, td, cellIndex, record, tr, rowIndex, e, eOpts) {
                    if (cellIndex == 3) {
                        opencenterwin("InWarehouseEdit_New.aspx?op=view&id=" + record.get("Id"), 630, 550);
                    }
                }
                }
            })
        ...
        function displayInnerGrid(div) {
            var store_inner = Ext.create('Ext.data.JsonStore', {
                fields: ['Id', 'InWarehouseId', 'ProductId', 'ProductCode', 'IQuantity', 'PurchaseOrderDetailId', 'YiRuQuan',
                    'InWarehouseState', 'Remark', 'SkinArray', 'SeriesArray', 'PurchaseOrderNo', 'FuKuanDanQuan', 'KaiPiaoQuan'],
                proxy: {
                    url: 'InWarehouseList_New.aspx?action=loaddetail&id=' + div,
                    type: 'ajax',
                    reader: {
                        type: 'json',
                        root: 'innerrows'
            ...
            var grid_inner = Ext.create('Ext.grid.Panel', {
                ...
                columns: [
                    ...
                    { header: '采购单号', dataIndex: 'PurchaseOrderNo', width: 110 },
                    { header: '产品型号', dataIndex: 'ProductCode', width: 160 },
                    { dataIndex: 'InWarehouseState', header: '入库状态', width: 70 },
                    { dataIndex: 'IQuantity', header: '入库数量', width: 70 },
                    { dataIndex: 'YiRuQuan', header: '已入库数量', width: 80 },
                    { header: '生成付款单数量', dataIndex: 'FuKuanDanQuan', width: 100 },
                    { header: '已关联发票数量', dataIndex: 'KaiPiaoQuan', width: 100 },
                    { dataIndex: 'Remark', header: '备注', width: 150 }
                    ],
                renderTo: div
            })
        }
