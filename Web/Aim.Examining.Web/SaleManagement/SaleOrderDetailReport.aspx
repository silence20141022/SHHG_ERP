<%@ Page Title="销售明细报表" Language="C#" AutoEventWireup="true" MasterPageFile="~/Masters/Ext/Site.Master"
    CodeBehind="SaleOrderDetailReport.aspx.cs" Inherits="Aim.Examining.Web.SaleManagement.SaleOrderDetailReport" %>
    ...
    <script type="text/javascript">
        ...
        function setPgUI() {
            ...
            store = new Ext.ux.data.AimJsonStore({
			    ...
			    { name: 'Pcn' }, { name: 'Count' }, { name: 'Amount' }, { name: 'Unit' }, { name: 'SalePrice' }, { name: 'OutCount' }, { name: 'OId' }, { name: 'CustomerOrderNo' },
			    { name: 'CreateTime' }, { name: 'Salesman' }, { name: 'ProfitRate' }, { name: 'BuyPrice' }, { name: 'EndDate' },
                { name: 'Rate' }, { name: 'CostPrice' }
			    ],
            ...
            tlBar = new Ext.ux.AimToolbar({
                ...
                { text: '压缩机', iconCls: 'aim-icon-search',
                 ...
                 { text: '工具', iconCls: 'aim-icon-search',
                     handler: function() {
                         productType = '工具'; $("#label1").text("当前显示的产品类型：" + productType);
                         store.reload();
                     }
            ...
            var columnsarray = [
                    ...
                    { id: 'OId', dataIndex: 'Id', header: '标识2', hidden: true },
					...
					{ id: 'Salesman', dataIndex: 'Salesman', header: '销售负责人', width: 80 },
					...
					{ id: 'Pcn', dataIndex: 'Pcn', header: 'PCN', width: 130, sortable: true },
                   	{ id: 'SalePrice', dataIndex: 'SalePrice', header: '售价', width: 70, sortable: true, renderer: RowRender,
					...
					{ id: 'Count', dataIndex: 'Count', header: '数量', width: 60, sortable: true, summaryType: 'sum' },
             		...
             		{ id: 'CustomerOrderNo', dataIndex: 'CustomerOrderNo', header: '客户订单号', width: 100 },
             ...
             if (AimState["UserInfo"].Name == "周俊青" || AimState["UserInfo"].Name == "缪玥莉" || AimState["UserInfo"].Name == "叶敏") {
                        columnsarray.push({ id: 'BuyPrice', dataIndex: 'BuyPrice', header: '采购价',renderer: RowRender});
                        columnsarray.push({ id: 'CostPrice', dataIndex: 'CostPrice', header: '成本价' });
                        columnsarray.push({ id: 'ProfitRate', dataIndex: 'ProfitRate', header: '利润率', width: 100, sortable: true,
                            renderer: RowRender, summaryType: 'average', summaryRenderer: function(v, params, data) {
                        var temp = Math.round(parseFloat(v) * 10000) / 100;
                        return temp + '%';
                    }
                });
            }
        ...
        function RowRender(value, cellmeta, record, rowIndex, columnIndex, store) {
            ...
            switch (this.id) {
                case "Number":
                    if (value) {
                        rtn = "<label style='color:Blue; cursor:pointer; text-decoration:underline;' onclick='window.open(\"SaleOrderView.aspx?id=" +
