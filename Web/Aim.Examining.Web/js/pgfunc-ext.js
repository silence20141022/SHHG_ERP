...
Ext.FlashComponent.EXPRESS_INSTALL_URL = '/App_Themes/Ext/expressinstall.swf';
Ext.chart.Chart.CHART_URL = '/App_Themes/Ext/charts.swf';
...
function ExtGridFileRender(val, p, rec) {
    ...
    var FileLinkBlock = "<a href='" + DOWNLOAD_PAGE_URL + "?Id={fileid}' style='margin:5px;' title='{filename}' "
            + " >{filename}</a>";
...
function ExtGridpperCase(val, p, rec) {
    if (val != null) {
...
Ext.ux.grid.AimCheckColumn = Ext.extend(Ext.ux.grid.CheckColumn, {
    constructor: function(config) {
        config = config || {};
        config.width = config.width || 60;

        Ext.ux.grid.AimCheckColumn.superclass.constructor.call(this, Ext.apply(config, {
    }));
},
...
Ext.reg('aimcheckcolumn', Ext.ux.grid.AimCheckColumn);
            ...
            if (config.items && $.isArray(config.items)) {
                ...
                for (var i = 0, pi = 0; i < config.items.length; i++) {
                    ...
                    if (i == 0 || (tflexcount + tfitem.flex) > pi) {
                        tautoitem = { layout: 'hbox', defaults: { margins: '0 0 0 0' }, listeners: { afterlayout: function(f) { f.el.dom.firstChild.firstChild.style.border = "1 solid transparent" } }, items: [] };
...
//------------------------Aim ExtJs 商品唯一编号选择控件 开始------------------------//

Ext.ux.AimProGuidSelector = Ext.extend(Ext.ux.form.AimPopup, {
    constructor: function(config) {
        config.popUrl = "/CommonPages/Select/ProductGuidSelect.aspx?rtntype=array";
        config.seltype = config.seltype || "multi";
        config.popUrl += "&seltype=" + config.seltype;

        config.popAfter = config.popAfter || function(rtn) {
            if (rtn && rtn.data) {
                this.setValue(rtn.data.GuId || "");
            }
        }

        Ext.ux.AimProGuidSelector.superclass.constructor.call(this, Ext.apply(config, {
    }));
}
});

Ext.reg('aimproguidselector', Ext.ux.AimProGuidSelector);

//------------------------Aim ExtJs 商品唯一编号选择控件 结束------------------------//
        ...
        if (this.popModel.toLowerCase() == "window")
            ...
            if (rtnval && rtnval.data) {//默认更新控件上的值
                if (this.seltype == "single") {
                    this.setValueEx([{ UserID: rtnval.data.UserID, Name: rtnval.data.Name}]);
                }
                else {
...
//------------------------Aim ExtJs Customer add by cc 控件 开始------------------------//
var customerselstore = new Ext.data.JsonStore({
    id: 'Id',
    root: 'rows',
    fields: [{ name: 'Id' }, { name: 'Name'}],
    url: '/commonpages/data/customerdata.aspx',
    baseParams: { cmd: 'GetCustomer' }
});
Ext.ux.form.AimCustomer = Ext.extend(Ext.ux.form.SuperBoxSelect, {
    store: customerselstore,
    popUrl: null,
    bodyStyle: 'title:expression(this.innerText)',
    popParam: null,
    popStyle: null,
    ...
    addNewDataOnBlur: true,
    clearBtnCls: 'aimuser-btn-clear',
    expandBtnCls: 'aimcustomer-btn-expand',
    minChars: 1,
    anchor: '100%',
    mode: 'remote',
    displayField: 'Name',
    ...
    resizable: false,
    constructor: function(config) {
        this.popUrl = "/CommonPages/Select/CustomerSelect.aspx?rtntype=array";
        this.seltype = config.seltype || "single";
        this.popUrl += "&seltype=" + this.seltype;
        this.popParam = config.popParam;
        this.popStyle = config.popStyle || "dialogWidth:750px; dialogHeight:550px; scroll:yes; center:yes; status:no; resizable:yes;";
        ...
        Ext.ux.form.AimCustomer.superclass.constructor.call(this, Ext.apply(config, {}));
        this.on("beforeadditem", function(bs, v) {
            if (bs.seltype == "single") {
                bs.reset();
                bs.addItem(v);
    ...
    onTriggerClick: function() {
        if (this.popModel.toLowerCase() == "window")
            ...
            if (rtnval && rtnval.data) {//默认更新控件上的值
                if (this.seltype == "single") {
                    this.setValueEx([{ Id: rtnval.data.Id, Name: rtnval.data.Name}]);
                }
                else {
                    for (var i = 0; i < rtnval.data.length; i++) {
                        this.addItem({ Id: rtnval.data[i].Id, Name: rtnval.data[i].Name });
                    }
                }
            }
        }
...
Ext.reg('aimcustomer', Ext.ux.form.AimCustomer);

//------------------------Aim ExtJs Customer控件 结束------------------------//



//------------------------Aim ExtJs Supplier add by cc 控件 开始------------------------//
var supplierselstore = new Ext.data.JsonStore({
    id: 'Id',
    root: 'rows',
    fields: [{ name: 'Id' }, { name: 'SupplierName'}],
    url: '/commonpages/data/SupplierData.aspx',
    baseParams: { cmd: 'GetSupplier' }
});
Ext.ux.form.AimSupplier = Ext.extend(Ext.ux.form.SuperBoxSelect, {
    store: supplierselstore,
    popUrl: null,
    bodyStyle: 'title:expression(this.innerText)',
    popParam: null,
    popStyle: null,
    ...
    displayFieldTpl: '{SupplierName}',
    valueField: 'Id',
    queryDelay: 30,
    triggerAction: 'all',
    resizable: false,
    constructor: function(config) {
        this.popUrl = "/CommonPages/Select/SupplierSelect.aspx?rtntype=array";
        this.seltype = config.seltype || "single";
        this.popUrl += "&seltype=" + this.seltype;
        this.popParam = config.popParam;
        this.popStyle = config.popStyle || "dialogWidth:750px; dialogHeight:550px; scroll:yes; center:yes; status:no; resizable:yes;";
        ...
        Ext.ux.form.AimSupplier.superclass.constructor.call(this, Ext.apply(config, {}));
        this.on("beforeadditem", function(bs, v) {
            if (bs.seltype == "single") {
                bs.reset();
                bs.addItem(v);
    ...
    onTriggerClick: function() {
        if (this.popModel.toLowerCase() == "window")
            ...
            if (rtnval && rtnval.data) {//默认更新控件上的值
                if (this.seltype == "single") {
                    this.setValueEx([{ Id: rtnval.data.Id, SupplierName: rtnval.data.SupplierName}]);
                }
                else {
                    for (var i = 0; i < rtnval.data.length; i++) {
                        this.addItem({ Id: rtnval.data[i].Id, SupplierName: rtnval.data[i].SupplierName, Symbo: rtnval.data[i].Symbo });
                    }
                }
            }
        }
...
Ext.reg('aimsupplier', Ext.ux.form.AimSupplier);

//------------------------Aim ExtJs Supplier控件 结束------------------------//

//------------------------Aim ExtJs 币种选择控件 MoneyType   add by PH 控件 开始------------------------//
var moneytypeselstore = new Ext.data.JsonStore({
    id: 'Id',
    root: 'rows',
    fields: [{ name: 'Id' }, { name: 'MoneyType'}],
    url: '/commonpages/data/MoneyTypeData.aspx',
    baseParams: { cmd: 'GetMoneyType' }
});
Ext.ux.form.AimMoneyType = Ext.extend(Ext.ux.form.SuperBoxSelect, {
    store: moneytypeselstore,
    popUrl: null,
    bodyStyle: 'title:expression(this.innerText)',
    popParam: null,
    popStyle: null,
    ...
    anchor: '100%',
    mode: 'remote',
    displayField: 'MoneyType',
    displayFieldTpl: '{MoneyType}',
    valueField: 'Id',
    queryDelay: 30,
    triggerAction: 'all',
    resizable: false,
    constructor: function(config) {
        this.popUrl = "/CommonPages/Select/ExchangeRateSelect.aspx?rtntype=array";
        this.seltype = config.seltype || "single";
        this.popUrl += "&seltype=" + this.seltype;
        this.popParam = config.popParam;
        this.popStyle = config.popStyle || "dialogWidth:750px; dialogHeight:550px; scroll:yes; center:yes; status:no; resizable:yes;";
    ...
    onTriggerClick: function() {
        if (this.popModel.toLowerCase() == "window")
            ...
            if (rtnval && rtnval.data) {//默认更新控件上的值
                if (this.seltype == "single") {
                    this.setValueEx([{ Id: rtnval.data.Id, MoneyType: rtnval.data.MoneyType}]);
                }
                else {
                    for (var i = 0; i < rtnval.data.length; i++) {
                        this.addItem({ Id: rtnval.data[i].Id, MoneyType: rtnval.data[i].MoneyType });
                    }
                }
            }
        }
...
Ext.reg('aimmoneytype', Ext.ux.form.AimMoneyType);

//------------------------Aim ExtJs MoneyType控件 结束------------------------//
